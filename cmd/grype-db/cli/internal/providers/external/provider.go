...
func (p pvdr) Update(ctx context.Context) error {
	if err := run(ctx, p.cfg.Cmd, p.cfg.ExecDir, p.Name(), p.cfg.Env); err != nil {
		return fmt.Errorf("failed to pull data from %q provider: %w", p.name, err)
	}
	return nil
}

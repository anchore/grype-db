...
func NewProvider(root string, name string, cfg Config) provider.Provider {
	return external.NewProvider(root, name,
		external.Config{
			Cmd:   getRunCommand(root, name, cfg),
			State: fmt.Sprintf("%s/metadata.json", name),
			Env:   cfg.Env,
		},
	)
}
...
func getRunCommand(root string, name string, cfg Config) string {
	switch cfg.Executor {
	case "docker", "podman":
		dataRootCtr := root
		...
		if _, err := os.Stat(".vunnel.yaml"); !os.IsNotExist(err) {
			cwd, err := os.Getwd()
			if err != nil {
				log.WithFields("error", err, "provider", name).Warn("unable to get current working directory, ignoring vunnel config")
			} else {
		...
		return fmt.Sprintf("%s run --rm -t -v %s:/%s %s %s %s:%s run %s", cfg.Executor, dataRootHost, dataRootCtr, cfgVol, envStr, cfg.DockerImage, cfg.DockerTag, name)
	}
	...
	return fmt.Sprintf("vunnel %s run %s", cfgSection, name)
}
...
func GenerateNames(root string, cfg Config) ([]string, error) {

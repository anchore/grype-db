...
check-schema: ## Ensure that there aren't any accidental changes to the DB schema version
	@ grep -qHr "const SchemaVersion" pkg || echo "SchemaVersion constant not found, check must be udpated to verify proper version"
	@ grep -r "const SchemaVersion" pkg | grep -q "= $(DATABASE_SCHEMA_VERSION)" || echo "Expected database schema version $(DATABASE_SCHEMA_VERSION) not found. Ensure 'const SchemaVersion = $(DATABASE_SCHEMA_VERSION)' is defined"

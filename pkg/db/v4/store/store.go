...
import (
	...
	"github.com/anchore/grype-db/pkg/db/internal/gormadapter"
	v4 "github.com/anchore/grype-db/pkg/db/v4"
	"github.com/anchore/grype-db/pkg/db/v4/store/model"
)
...
func (s *store) AddVulnerabilityMetadata(metadata ...v4.VulnerabilityMetadata) error {
	for _, m := range metadata {
		...
		incoming:
			...
			links := strset.New(existing.URLs...)
			for _, l := range m.URLs {
				links.Add(l)
			}

			existing.URLs = links.List()

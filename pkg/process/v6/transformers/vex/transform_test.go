package vex

import (
	"reflect"
	"testing"
	"time"

	"github.com/google/go-cmp/cmp"
	govex "github.com/openvex/go-vex/pkg/vex"
	"github.com/stretchr/testify/require"

	"github.com/anchore/grype-db/pkg/process/v6/transformers"
	"github.com/anchore/grype-db/pkg/provider"
	"github.com/anchore/grype-db/pkg/provider/unmarshal"
	grypeDB "github.com/anchore/grype/grype/db/v6"
	"github.com/anchore/packageurl-go"
)

var timeVal = time.Date(2021, 1, 1, 0, 0, 0, 0, time.UTC)
var listing = provider.File{
	Path:      "some",
	Digest:    "123456",
	Algorithm: "sha256",
}

func inputProviderState() provider.State {
	return provider.State{
		Provider:  "vex",
		Version:   1,
		Processor: "vunnel@1.2.3",
		Timestamp: timeVal,
		Listing:   &listing,
	}
}

func Test_getGrypeRanges(t *testing.T) {
	tests := []struct {
		name   string
		purl   *packageurl.PackageURL
		status govex.Status
		want   []grypeDB.AffectedRange
	}{
		{
			name: "fixed vuln",
			// pkg:pypi/urllib3@1.26.16+cgr.1
			purl: &packageurl.PackageURL{
				Name:    "urllib3",
				Type:    "pypi",
				Version: "1.26.16+cgr.1",
			},
			status: govex.StatusFixed,
			want: []grypeDB.AffectedRange{
				{
					Version: grypeDB.AffectedVersion{
						Type:       "semver",
						Constraint: "< 1.26.16+cgr.1",
					},
				},
				{
					Fix: &grypeDB.Fix{
						Version: "1.26.16+cgr.1",
						State:   grypeDB.FixedStatus,
					},
				},
			},
		},
		{
			name: "unaffected vuln",
			// pkg:pypi/urllib3@1.24.16+cgr.1
			purl: &packageurl.PackageURL{
				Name:    "urllib3",
				Type:    "pypi",
				Version: "1.24.16+cgr.1",
			},
			status: govex.StatusNotAffected,
			want: []grypeDB.AffectedRange{{
				Fix: &grypeDB.Fix{
					Version: "1.24.16+cgr.1",
					State:   grypeDB.NotAffectedFixStatus,
				},
			}},
		},
		{
			name: "affected vuln",
			// pkg:pypi/urllib3@1.23.13+cgr.1
			purl: &packageurl.PackageURL{
				Name:    "urllib3",
				Type:    "pypi",
				Version: "1.23.13+cgr.1",
			},
			status: govex.StatusAffected,
			want: []grypeDB.AffectedRange{{
				Version: grypeDB.AffectedVersion{
					Type:       "semver",
					Constraint: "== 1.23.13+cgr.1",
				},
			}},
		},
	}
	for _, testToRun := range tests {
		test := testToRun
		t.Run(test.name, func(tt *testing.T) {
			if got := getGrypeRanges(test.purl, test.status); !reflect.DeepEqual(got, test.want) {
				t.Errorf("getGrypeRanges() = %v, want %v", got, test.want)
			}
		})
	}
}

func Test_GetAffectedPackages(t *testing.T) {
	tests := []struct {
		name    string
		vuln    unmarshal.VEXVulnerability
		want    []grypeDB.AffectedPackageHandle
		wantErr bool
	}{
		{
			name: "empty products returns nil",
			vuln: govex.Statement{
				ID:       "test-vuln-1",
				Products: []govex.Product{},
				Status:   govex.StatusAffected,
			},
			want:    nil,
			wantErr: false,
		},
		{
			name: "single affected product",
			vuln: govex.Statement{
				ID: "test-vuln-2",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "pkg:pypi/urllib3@1.26.16",
						},
					},
				},
				Status: govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:      "CVE-2023-43804",
					Aliases: []govex.VulnerabilityID{"GHSA-v845-jxx5-vc9f"},
				},
			},
			want: []grypeDB.AffectedPackageHandle{
				{
					Package: &grypeDB.Package{
						Ecosystem: "pypi",
						Name:      "urllib3",
					},
					BlobValue: &grypeDB.AffectedPackageBlob{
						CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
						Ranges: []grypeDB.AffectedRange{
							{
								Version: grypeDB.AffectedVersion{
									Type:       "semver",
									Constraint: "== 1.26.16",
								},
							},
						},
					},
				},
			},
			wantErr: false,
		},
		{
			name: "multiple products sorted alphabetically",
			vuln: govex.Statement{
				ID: "test-vuln-3",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "pkg:pypi/urllib3@1.26.16",
						},
					},
					{
						Component: govex.Component{
							ID: "pkg:npm/express@4.18.2",
						},
					},
				},
				Status: govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:      "CVE-2023-43804",
					Aliases: []govex.VulnerabilityID{"GHSA-v845-jxx5-vc9f"},
				},
			},
			want: []grypeDB.AffectedPackageHandle{
				{
					Package: &grypeDB.Package{
						Ecosystem: "npm",
						Name:      "express",
					},
					BlobValue: &grypeDB.AffectedPackageBlob{
						CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
						Ranges: []grypeDB.AffectedRange{
							{
								Version: grypeDB.AffectedVersion{
									Type:       "semver",
									Constraint: "== 4.18.2",
								},
							},
						},
					},
				},
				{
					Package: &grypeDB.Package{
						Ecosystem: "pypi",
						Name:      "urllib3",
					},
					BlobValue: &grypeDB.AffectedPackageBlob{
						CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
						Ranges: []grypeDB.AffectedRange{
							{
								Version: grypeDB.AffectedVersion{
									Type:       "semver",
									Constraint: "== 1.26.16",
								},
							},
						},
					},
				},
			},
			wantErr: false,
		},
		{
			name: "fixed status product",
			vuln: govex.Statement{
				ID: "test-vuln-4",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "pkg:pypi/urllib3@2.0.7",
						},
					},
				},
				Status: govex.StatusFixed,
				Vulnerability: govex.Vulnerability{
					ID:      "CVE-2023-43804",
					Aliases: []govex.VulnerabilityID{"GHSA-v845-jxx5-vc9f"},
				},
			},
			want: []grypeDB.AffectedPackageHandle{
				{
					Package: &grypeDB.Package{
						Ecosystem: "pypi",
						Name:      "urllib3",
					},
					BlobValue: &grypeDB.AffectedPackageBlob{
						CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
						Ranges: []grypeDB.AffectedRange{
							{
								Version: grypeDB.AffectedVersion{
									Type:       "semver",
									Constraint: "< 2.0.7",
								},
							},
							{
								Fix: &grypeDB.Fix{
									Version: "2.0.7",
									State:   grypeDB.FixedStatus,
								},
							},
						},
					},
				},
			},
			wantErr: false,
		},
		{
			name: "invalid purl returns error",
			vuln: govex.Statement{
				ID: "test-vuln-5",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "invalid-purl",
						},
					},
				},
				Status: govex.StatusAffected,
			},
			want:    nil,
			wantErr: true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := getAffectedPackages(tt.vuln)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
			if diff := cmp.Diff(tt.want, got); diff != "" {
				t.Errorf("GetAffectedPackages() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

func TestVEXTransform(t *testing.T) {
	tests := []struct {
		name    string
		state   provider.State
		vuln    unmarshal.VEXVulnerability
		wantErr bool
		want    transformers.RelatedEntries
	}{
		{
			name:  "basic VEX vulnerability with single product",
			state: inputProviderState(),
			vuln: govex.Statement{
				ID: "test-transform-1",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "pkg:pypi/urllib3@1.26.16",
						},
					},
				},
				Status: govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:          "CVE-2023-43804",
					Description: "urllib3 HTTP Request Smuggling vulnerability",
					Aliases:     []govex.VulnerabilityID{"GHSA-v845-jxx5-vc9f"},
				},
			},
			want: transformers.RelatedEntries{
				VulnerabilityHandle: &grypeDB.VulnerabilityHandle{
					Name:       "CVE-2023-43804",
					Status:     grypeDB.VulnerabilityActive,
					ProviderID: "vex",
					Provider: &grypeDB.Provider{
						ID:           "vex",
						Version:      "1",
						Processor:    "vunnel@1.2.3",
						DateCaptured: &timeVal,
						InputDigest:  "sha256:123456",
					},
					BlobValue: &grypeDB.VulnerabilityBlob{
						ID:          "CVE-2023-43804",
						Description: "urllib3 HTTP Request Smuggling vulnerability",
						References: []grypeDB.Reference{
							{
								URL: "CVE-2023-43804",
							},
						},
						Aliases: []string{"GHSA-v845-jxx5-vc9f"},
					},
				},
				Related: []any{
					grypeDB.AffectedPackageHandle{
						Package: &grypeDB.Package{
							Ecosystem: "pypi",
							Name:      "urllib3",
						},
						BlobValue: &grypeDB.AffectedPackageBlob{
							CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
							Ranges: []grypeDB.AffectedRange{
								{
									Version: grypeDB.AffectedVersion{
										Type:       "semver",
										Constraint: "== 1.26.16",
									},
								},
							},
						},
					},
				},
			},
		},
		{
			name:  "VEX vulnerability with multiple products",
			state: inputProviderState(),
			vuln: govex.Statement{
				ID: "test-transform-2",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "pkg:pypi/urllib3@1.26.16",
						},
					},
					{
						Component: govex.Component{
							ID: "pkg:npm/express@4.18.2",
						},
					},
				},
				Status: govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:          "CVE-2023-43804",
					Description: "Test vulnerability affecting multiple packages",
					Aliases:     []govex.VulnerabilityID{"GHSA-v845-jxx5-vc9f"},
				},
			},
			want: transformers.RelatedEntries{
				VulnerabilityHandle: &grypeDB.VulnerabilityHandle{
					Name:       "CVE-2023-43804",
					Status:     grypeDB.VulnerabilityActive,
					ProviderID: "vex",
					Provider: &grypeDB.Provider{
						ID:           "vex",
						Version:      "1",
						Processor:    "vunnel@1.2.3",
						DateCaptured: &timeVal,
						InputDigest:  "sha256:123456",
					},
					BlobValue: &grypeDB.VulnerabilityBlob{
						ID:          "CVE-2023-43804",
						Description: "Test vulnerability affecting multiple packages",
						References: []grypeDB.Reference{
							{
								URL: "CVE-2023-43804",
							},
						},
						Aliases: []string{"GHSA-v845-jxx5-vc9f"},
					},
				},
				Related: []any{
					grypeDB.AffectedPackageHandle{
						Package: &grypeDB.Package{
							Ecosystem: "npm",
							Name:      "express",
						},
						BlobValue: &grypeDB.AffectedPackageBlob{
							CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
							Ranges: []grypeDB.AffectedRange{
								{
									Version: grypeDB.AffectedVersion{
										Type:       "semver",
										Constraint: "== 4.18.2",
									},
								},
							},
						},
					},
					grypeDB.AffectedPackageHandle{
						Package: &grypeDB.Package{
							Ecosystem: "pypi",
							Name:      "urllib3",
						},
						BlobValue: &grypeDB.AffectedPackageBlob{
							CVEs: []string{"CVE-2023-43804", "GHSA-v845-jxx5-vc9f"},
							Ranges: []grypeDB.AffectedRange{
								{
									Version: grypeDB.AffectedVersion{
										Type:       "semver",
										Constraint: "== 1.26.16",
									},
								},
							},
						},
					},
				},
			},
		},
		{
			name:  "VEX vulnerability with no products",
			state: inputProviderState(),
			vuln: govex.Statement{
				ID:       "test-transform-3",
				Products: []govex.Product{},
				Status:   govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:          "CVE-2023-43804",
					Description: "Test vulnerability with no products",
				},
			},
			want: transformers.RelatedEntries{
				VulnerabilityHandle: &grypeDB.VulnerabilityHandle{
					Name:       "CVE-2023-43804",
					Status:     grypeDB.VulnerabilityActive,
					ProviderID: "vex",
					Provider: &grypeDB.Provider{
						ID:           "vex",
						Version:      "1",
						Processor:    "vunnel@1.2.3",
						DateCaptured: &timeVal,
						InputDigest:  "sha256:123456",
					},
					BlobValue: &grypeDB.VulnerabilityBlob{
						ID:          "CVE-2023-43804",
						Description: "Test vulnerability with no products",
						References: []grypeDB.Reference{
							{
								URL: "CVE-2023-43804",
							},
						},
						Aliases: []string{},
					},
				},
			},
		},
		{
			name:  "VEX vulnerability with invalid product purl",
			state: inputProviderState(),
			vuln: govex.Statement{
				ID: "test-transform-4",
				Products: []govex.Product{
					{
						Component: govex.Component{
							ID: "invalid-purl",
						},
					},
				},
				Status: govex.StatusAffected,
				Vulnerability: govex.Vulnerability{
					ID:          "CVE-2023-43804",
					Description: "Test vulnerability with invalid purl",
				},
			},
			wantErr: true,
		},
	}

	t.Parallel()
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := Transform(tt.vuln, tt.state)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
			require.Len(t, got, 1, "should return exactly one RelatedEntries")

			e, ok := got[0].Data.(transformers.RelatedEntries)
			require.True(t, ok)
			if diff := cmp.Diff(tt.want, e); diff != "" {
				t.Errorf("data entries mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
